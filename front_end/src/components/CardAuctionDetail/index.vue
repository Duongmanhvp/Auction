            ...
            <div class="flex space-x-8">
                <div class="w-1/2 relative">
                    <div class="w-full h-auto overflow-hidden rounded-md">
                        <div class="flex-1">
                                <img v-for="(image, index) in arrayImage" :key="index" :src="image"
                                    alt="Image" v-show="index === currentImageIndex" class="h-max w-max" />
                    ...
                    <button v-if=" auction.status === 'OPENING'" @click="handleRegister(auction.id)" class="bg-green-500 text-white p-2 rounded mt-2 w-full">
                        Register Auction 
                    </button>
                    <button v-if=" auction.status === 'IN_PROGRESS'" @click="handleJoin(auction.id)" class="bg-green-500 text-white p-2 rounded mt-2 w-full">
                        Join Auction
                    </button>
                </div>
...
const updateArrayImage = () => {
    if (props.auction) {
        arrayImage.value = props.auction.image.split(', ').map(img => `https://res.cloudinary.com/dorl0yxpe/image/upload/` + img.trim());
